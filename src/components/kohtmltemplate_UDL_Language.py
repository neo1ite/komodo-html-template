# Registers the htmltemplate language in Komodo.

import logging
from koXMLLanguageBase import koHTMLLanguageBase

log = logging.getLogger("kohtmltemplateLanguage")
#log.setLevel(logging.DEBUG)

def registerLanguage(registry):
    log.debug("Registering language htmltemplate")
    registry.registerLanguage(KohtmltemplateLanguage())

class KohtmltemplateLanguage(koHTMLLanguageBase):

    # ------------ Komodo Registration Information ------------ #

    name = "htmltemplate"
    lexresLangName = "htmltemplate"
    _reg_desc_ = "%s Language" % name
    _reg_contractid_ = "@activestate.com/koLanguage?language=%s;1" % name
    _reg_categories_ = [("komodo-language", name)]
    _reg_clsid_ = "89641253-7e2e-46e7-9576-1ad48248328e"
    ...
    #           'M': 'XML',             #   XML
    #        }
    lang_from_udl_family = {'SSL': 'htmltemplate', 'M': 'HTML'}
